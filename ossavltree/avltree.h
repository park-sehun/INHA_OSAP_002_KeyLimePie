...
public:
    ...
    void rebalance(node* current_node, int current_node_balance_factor); //root가 currnt_node인 서브트리에서 rotation을 실행하는 함수
    void rightRotate(node* node_z); //rightRotate를 실행하는 함수
    void leftRotate(node* node_z); //leftRotate를 실행하는 함수
    void insertPosition(node* parent_node, node* current_node, int& child_factor, int x); //노드를 추가할 위치를 구하는 함수
    void setChildNode(node* parent_node, node* child_node, int child_factor); //자식노드로 설정하는 함수
    int minimum(int x); //key값이 x인 노드가 루트인 서브트리에서의 최소 key를 가지는 노드의 key와 depth를 출력하는 함수
    int maximum(int x); //key값이 x인 노드가 루트인 서브트리에서의 최대 key를 가지는 노드의 key와 depth를 출력하는 함수
    int empty(); //tree가 비어있는지를 출력하는 함수
    int size(); //tree의 크기를 출력하는 함수
...
void AVLtree::balance(node* current_node) //root가 currnt_node인 서브트리에서 균형을 맞추는 함수
{
    ...
    rebalance(current_node, current_node_balance_factor);
    return;
}

void AVLtree::rebalance(node* current_node, int current_node_balance_factor) { //root가 currnt_node인 서브트리에서 rotation을 실행하는 함수
    if (current_node_balance_factor > 1) //왼쪽 자식 노드의 높이가 2이상으로 더 높은 경우
    {
        if (getBalanceFactor(current_node->left_child) > 1) // LL 변환
        {
            rightRotate(current_node);
        }
        else if (getBalanceFactor(current_node->left_child) < -1) // LR 변환
        {
    ...
    else if (current_node_balance_factor < -1) //오른쪽 자식 노드의 높이가 2이상으로 더 높은 경우
    {
        ...
        else if (getBalanceFactor(current_node->right_child) < -1) // RR 변환
        {
            leftRotate(current_node);          
        }
    }
...
void AVLtree::insertPosition(node* parent_node, node* current_node, int& child_factor , int x) { //노드를 추가할 위치를 구하는 함수
...
void AVLtree::setChildNode(node* parent_node, node* child_node, int child_factor) { //자식노드로 설정하는 함수
...
int AVLtree::minimum(int x) //key값이 x인 노드가 루트인 서브트리에서의 최소 key를 가지는 노드의 key와 depth를 출력하는 함수
{
    ...
    return minimum_node_depth;
}

int AVLtree::maximum(int x) //key값이 x인 노드가 루트인 서브트리에서의 최대 key를 가지는 노드의 key와 depth를 출력하는 함수
{
    ...
    return maximum_node_depth;
}

int AVLtree::empty() //tree가 비어있는지를 출력하는 함수
{
...
int AVLtree::size() //tree의 크기를 출력하는 함수
{
    return node_num;
}
...
int AVLtree::insert(int x) //key값이 x인 노드를 삽입하는 함수
{
    ...
    insertPosition(parent_node, current_node, child_factor, x);

    setChildNode(parent_node, current_node, child_factor);
...
#endif OSSAVLTREE_OSSAVLTREE_AVLTREE_H_
